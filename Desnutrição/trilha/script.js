/* ================== Config base ================== */
const unidade = "trilha"; // <── altere conforme a unidade atual (ex: un1, un2, un3...)
...
async function getPaginaHTML(numero, baseUrl = `paginas_trilha/pagina${numero}.html`, signal) {
...
async function carregarPagina(numero) {
  ...
  try {
    ...
    const html = await getPaginaHTML(numero, `paginas_trilha/pagina${numero}.html`, currentController.signal);
      ...
      rIC(async () => {
        try {
          ...
          if (!cachePaginas.has(proxNum)) {
            const respProx = await fetch(`paginas_trilha/pagina${proxNum}.html`);
...
prevBtn.addEventListener("click", () => {
  if (paginaAtual > 0) {
    ...
    const unidadeAnterior = "un4";
    const storageKeyAnterior = `paginaAtual_${unidadeAnterior}`;
    sessionStorage.setItem(storageKeyAnterior, 36); // ex: última página da un2
    window.location.href = `../unidade 4/${unidadeAnterior}.html`;
  }
});
